...
import pe.common.residue_constants as rc
...
class Protein:
  ...
  atom_positions: torch.Tensor  # [num_res, num_atom_type, 3]

  # Amino-acid type for each residue represented as an integer between 0 and
  # 20, where 20 is 'X'.
  aatype: torch.Tensor  # [num_res]

  # Binary float mask to indicate presence of a particular atom. 1.0 if an atom
  # is present and 0.0 if not. This should be used for loss masking.
  atom_mask: torch.Tensor  # [num_res, num_atom_type]

  # Residue index as used in PDB. It is not necessarily continuous or 0-indexed.
  residue_index: torch.Tensor  # [num_res]

  # B-factors, or temperature factors, of each residue (in sq. angstroms units),
  # representing the displacement of the residue from its ground truth mean
  # value.
  b_factors: torch.Tensor  # [num_res, num_atom_type]
...
def from_pdb_string(pdb_str: str, chain_id: Optional[str] = None) -> Protein:
  ...
  for res in chain:
    ...
    pos = torch.zeros(len(rc.main_chain_atom_types), 3)
    mask = torch.zeros(len(rc.main_chain_atom_types))
    res_b_factors = torch.zeros(len(rc.main_chain_atom_types))
    for atom in res:
      if atom.name not in rc.main_chain_atom_types:
        continue
      pos[rc.main_chain_atom_order[atom.name]] = torch.tensor(atom.coord)
      mask[rc.main_chain_atom_order[atom.name]] = 1.
      res_b_factors[rc.main_chain_atom_order[atom.name]] = atom.bfactor
  ...
  return Protein(
      atom_positions=torch.stack(atom_positions),
...
#   if torch.any(aatype > rc.restype_num):
